// L0 bug: zeEventPoolCreate hangs with concurrent zeEventHostSynchronize (Aurora, 12 devices)
...
int main(){
  ...
  for(int i=0;i<10;i++){ze_event_pool_handle_t p;C(zeEventPoolCreate(ctx,&pd,0,nullptr,&p));zeEventPoolDestroy(p);}
  stop=true;zeEventHostSignal(evt);t.join();
  zeEventDestroy(evt);zeEventPoolDestroy(pool);zeCommandListDestroy(cl);zeContextDestroy(ctx);
  std::cout<<"PASS\n";return 0;
}

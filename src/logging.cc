...
void _setupSpdlog() {
  spdlog::set_default_logger(spdlog::stderr_color_mt("HIPXX"));
  spdlog::set_level(spdlog::level::debug);
  spdlog::set_pattern("%n %^%l%$ [TID %t] [%E.%F] : %v");

  spdlog::level::level_enum spd_loglevel;

  const char *loglevel = getenv("HIPXX_LOGLEVEL");
  if (loglevel) {
    std::cout << "HIPXX_LOGLEVEL=" << loglevel << "\n";
    ...
    std::cout << "HIPXX_LOGLEVEL was not set. Default to trace\n";

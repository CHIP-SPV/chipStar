...
hipError_t __hipPopCallConfiguration(dim3* gridDim, dim3* blockDim,
  ...
  auto* ei = Backend->ChipExecStack.top();
  *gridDim = ei->getGrid();
...
hipError_t hipDeviceGetCacheConfig(hipFuncCache_t* cacheConfig) {
  ...
  if (cacheConfig)
    *cacheConfig = Backend->getActiveDevice()->getCacheConfig();
  RETURN(hipSuccess);
...
hipError_t hipDeviceGetSharedMemConfig(hipSharedMemConfig* pConfig) {
  ...
  if (pConfig)
    *pConfig = Backend->getActiveDevice()->getSharedMemConfig();
  RETURN(hipSuccess);
...
hipError_t hipDeviceGetStreamPriorityRange(int* leastPriority,
  ...
  if (leastPriority)
    *leastPriority = q->getPriorityRange(0);
  if (greatestPriority)
    *greatestPriority = q->getPriorityRange(1);
  RETURN(hipSuccess);
...
hipError_t hipLaunchByPtr(const void* hostFunction) {
  ...
  Backend->getActiveDevice()->initializeDeviceVariables();
  CHIPExecItem* exec_item = Backend->ChipExecStack.top();
  Backend->ChipExecStack.pop();
...
extern "C" hipError_t hipInitFromOutside(void* driverPtr, void* devicePtr,
  ...
  Backend->ChipContexts[0]->getDevices().push_back(chip_dev);

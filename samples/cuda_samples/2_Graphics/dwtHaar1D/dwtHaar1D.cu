...
runTest(int argc, char **argv)
{
    ...
    else
    {
        ...
        grid_size.x = (num_threads_total_left / 256);
        block_size.x = 256;

        // 512 threads corresponds to 10 decomposition steps
        // 256 threads corresponds to 9? decomposition steps
        dlevels_step = 9;
        dlevels_left -= 9;
    ...
    while (0 != num_threads_total_left)
    {
        ...
        dlevels_step = (dlevels_left > 9) ? dlevels_left - 9 : dlevels_left;
        dlevels_left -= 9;

        // after each step only half the threads are used any longer
        // therefore after 10 steps 2^10 less threads
        num_threads_total_left = num_threads_total_left >> 9;

        // update block and grid size
        grid_size.x = (num_threads_total_left / 256)
                      + (0 != (num_threads_total_left % 256)) ? 1 : 0;

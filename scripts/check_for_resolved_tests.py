...
parser.add_argument('backend', help='Backend to check for resolved tests', choices=['opencl', 'level0', 'pocl'])
...
os.chdir(args.path)
out = run_cmd("{env_vars} ctest --timeout 120 -j {num_threads}  -R \"`cat ./test_lists/{precise_device_type}_{backend}_failed_tests.txt`\"".format(backend=args.backend, precise_device_type=precise_device_type, env_vars=env_vars, num_threads=args.num_threads))

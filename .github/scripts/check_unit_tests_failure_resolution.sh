...
ctest -N -R "`cat ./igpu_opencl_passed.txt`" | tee igpu_opencl_flaky_tests.txt
CHIP_BE=opencl CHIP_DEVICE_TYPE=gpu CHIP_PLATFORM=4 CHIP_DEVICE=0 ctest --timeout 180 -j 8 --repeat until-fail:100 -R "`cat ./igpu_opencl_passed.txt`" | tee igpu_opencl_flaky_tests.txt
echo "end igpu_opencl_passed_tests"

# Test OpenCL dGPU
echo "begin dgpu_opencl_passed_tests"
ctest -N -R "`cat ./dgpu_opencl_passed.txt`" | tee dgpu_opencl_flaky_tests.txt
CHIP_BE=opencl CHIP_DEVICE_TYPE=gpu CHIP_PLATFORM=3 CHIP_DEVICE=0 ctest --timeout 180 -j 8 --repeat until-fail:100 -R "`cat ./dgpu_opencl_passed.txt`" | tee dgpu_opencl_flaky_tests.txt
echo "end dgpu_opencl_passed_tests"

# Test OpenCL CPU
echo "begin cpu_opencl_passed_tests"
ctest -N -R "`cat ./cpu_opencl_passed.txt`" | tee cpu_opencl_flaky_tests.txt
CHIP_BE=opencl CHIP_DEVICE_TYPE=cpu CHIP_PLATFORM=1 CHIP_DEVICE=0 ctest --timeout 180 -j 8 --repeat until-fail:100 -R "`cat ./cpu_opencl_passed.txt`" | tee cpu_opencl_flaky_tests.txt
echo "end cpu_opencl_passed_tests"

# Test Level Zero iGPU
echo "begin igpu_level0_passed_tests"
ctest -N -R "`cat ./igpu_level0_passed.txt`" | tee igpu_level0_flaky_tests.txt
CHIP_BE=level0 CHIP_DEVICE=1 ctest --timeout 180 -j 1 --repeat until-fail:100 -R "`cat ./igpu_level0_passed.txt`" | tee igpu_level0_flaky_tests.txt
